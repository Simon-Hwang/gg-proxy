# Per-User Upstream Credentials — Plan v2

**Status:** DRAFT (pending Santa dual-review, round 2)
**Date:** 2026-05-25
**Supersedes:** plan-v1 (Santa round 1 FAIL — see Appendix Z for what changed)
...
- `POST /api/v1/sessions/{sid}/retry` (router passes the
  retrier's `api_key_label`, not the original submitter — closes
...
| R7 | Multi-worker cache incoherence | v2 has NO cache — each submit hits DB. No incoherence to manage. Latency budget A.6.4 shows <2ms tax. |
...
## Appendix Z. v1 → v2 changelog (for reviewers who saw v1)
