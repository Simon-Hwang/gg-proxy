...
- [x] Task 0 spike 完成 + 报告写好 — `scripts/spike_docker_round_trip.sh` 7/7 pass; `docs/docker-runner-spike.md` 落地; D3.2/D3.7/D3.13/D3.15/D3.19 全部验证通过，无需调整决策（仅追加：DockerExecutor 须给容器加 `ExtraHosts: ["host.docker.internal:host-gateway"]`）。
- [x] **deps + SessionRuntimeContext** — `aiodocker>=0.21,<1.0` + `aiohttp>=3.9` 进 `pyproject.toml`；`SessionRuntimeContext` frozen+slots 落 `session/spec.py`；`ExecutorBackend.start(spec, *, runtime_ctx=…)` 引入；新增 `requires_docker` / `requires_curl` marker；5 个新测试，128→133 pass。
- [x] **Task 1** — `UnixSocketTransport` + `UnixSocketServer`（NDJSON over AF_UNIX, drain-then-close, accept-set tracked close）落地；12 个测试通过（plan 写 8，多加 4：connect-timeout / send-after-close / unlink-on-server-close / stale-file-replace）；145 total pass。
- [x] **Task 2 + Task 5（合并提交）** — `WireCoordinatorProxy` (`session/runner/proxy_client.py`) + `_make_runner_core` 提取 + `make_wire_runner` 落地；`wire_runner.py` 入口已含 Task 7 的 env validation + signal handler；`SessionSpec.to_json` / `from_json` 顺带补全（DockerExecutor 注入 spec_json env 必备）。**偏离**：Task 5 比 plan 顺序提前到 Task 2 之内（wire_runner 必须 import `make_wire_runner`，不抽取就跑不起来）；Task 7 也合并到 Task 2（wire_runner 单文件就涵盖了 sanity wrapper 的所有职责）。6 个 WireCoordinatorProxy 测试 + 7 个 wire_runner/spec 新测试，145→161 pass，mypy/ruff clean。
- [x] **Task 3** — `WireBridge` 落地 (`session/runner/bridge.py`)；含 tool.request 路由、ping/pong、shutdown grace、tool task 取消、coordinator 异常 → deny 兜底；新增 `make_tool_decision` / `make_ping` / `make_pong` / `make_shutdown` builders；11 个测试通过（plan 写 6，多加 5：close-exits-run / coordinator-failure / shutdown-no-response / shutdown-idempotent / parametrised finished-flag），161→172 pass。
- [x] **Task 4** — `DockerExecutor` (`session/executor/docker.py`) 落地，aiodocker 客户端依赖注入（生产用 lazy `Docker()`；测试用 MagicMock + AsyncMock）。HostConfig 严格落实 D3.6/D3.7/D3.16（mem/cpus/pids、bridge+host-gateway extra_hosts、`:z` mount）。start() 三段式：bind socket → docker run → server.accept；任一段失败都干净回滚；stop() 是 graceful (shutdown frame + 5s wait) → kill 兜底；health() = container.show().Status=="running" ∧ transport.is_alive。15 个单元测试（plan 写 7；多加 _parse_size 6 个参数化、close-idempotent、accept-timeout 等），172→187 pass。
- [x] **Task 6** — `images/gg-relay-runner/{Dockerfile, .dockerignore, README.md}` 落地。Multi-stage: plugins-builder (`node:20-bookworm-slim` + git clone gg-plugins tag + install.sh `--profile full`) → runtime (`python:3.11-slim` + tini + nodejs 20 + claude CLI pin + gg-relay editable install)。Non-root `gguser` UID 1000、healthcheck = `claude --version`、ENTRYPOINT=`tini --`、CMD=`python -m gg_relay.session.runner.wire_runner`。**未本地 docker build**（plan 明确说可跳）；信任 Dockerfile + Task 0 spike 已证明 4 件套构建可行。README 含 build args / 调试命令 / 必需 mount layout / SELinux 排错 / 镜像大小预期。
- [x] **Task 8** — `.github/workflows/build-runner-image.yml` 落地。触发：`workflow_dispatch` (gg_plugins_version + claude_cli_version + profile) + `repository_dispatch[gg_plugins_release]`（gg-plugins CI 侧 curl 触发，payload 带 tag/cli/profile）。inputs/payload 两侧 fallback。GHCR 登录 + buildx + cache (`type=gha,scope=gg-relay-runner`) + 双 tag (`gg-plugins-${ver}` + `latest`) + step summary 表格。YAML 用 PyYAML safe_load 校验通过。
- [x] **Task 9** — heartbeat 完整闭环。`WireBridge` 新增 `_heartbeat_loop` 周期发 `ping`（D3.10），收到 `pong` 清零 miss 计数；连续 `heartbeat_misses_before_unhealthy=3` 未收 pong → `heartbeat_unhealthy=True` + 缓冲 `error code=heartbeat_timeout` 帧 + 触发用户 callback。`WireCoordinatorProxy.consume_loop` 收到 `ping` 自动回 `pong`（Plan 3 D3.10 容器侧反应式语义）。**协议拓扑修正**：把原 Task 3 测试里"runner 发 ping，bridge 回 pong"反向（Plan 协议明确 PingFrame 是 host→runner，PongFrame 是 runner→host），相应改写测试。新增 4 个 heartbeat 测试 + 1 个 ping→pong 端到端集成测试。187→191 pass。
- [x] **Task 10** — `tests/integration/test_docker_executor.py` 落地，`@pytest.mark.requires_docker @pytest.mark.requires_api_key` 双 marker；本环境 deselect 跳过（无 API key）。test 完整：build/reuse `gg-relay-runner:dev` 镜像 → `DockerExecutor.start` → `WireBridge.run` → 等 session.end → stop。`GG_RELAY_RUNNER_IMAGE` env 可覆盖镜像 tag。
- [x] **Task 12** — `src/gg_relay/proxy/{__init__.py, audit.py, server.py}` 落地：`MinimalProxy` 用 **raw asyncio.start_server** 实现（aiohttp.web 不原生支持 HTTP CONNECT，已在 server.py docstring 顶部说明替换原因），覆盖 CONNECT + plain-HTTP 双路径、白名单（D3.13 `frozenset({"api.anthropic.com"})` 默认）、`X-Relay-Session-Id` 解析、500ms grace 写入、双向 pipe、upstream 不可达 → 502。`AuditLog` 是线程安全的 JSONL append（`threading.RLock` + `asyncio.to_thread`），allow/deny 都带 ISO8601 `ts`。9 个单元测试（plan 写 6，实际多加 audit-allow / audit-deny / 异步上下文管理 / 双 close 幂等）+ 1 个 curl 集成 smoke（`@requires_curl`，curl 在该环境可用，已实跑通过）。191→200 pass，全 mypy/ruff clean。
- [x] **Task 11** — Coverage + spec sync + 新 demo 落地。 (a) `examples/docker_executor_demo.py` 写好双模 (`DOCKER_AVAILABLE=true` → 真 DockerExecutor；`DOCKER_AVAILABLE=false` → 同进程 `_stub_runner_side` 走真实 UnixSocketTransport pair，验证 WireBridge ↔ WireCoordinatorProxy 端到端)，stub 模式本地 200 OK 实跑（Write 自动放行 + Bash HITL approve + session.end 完整时序）。(b) `examples/walking_skeleton_demo.py` 复测仍 exit 0，Task 5 refactor 无回归。(c) spec 同步：§5.4 `DockerExecutor` + §5.5 `WireBridge/WireCoordinatorProxy` + §6.6 `UnixSocketTransport` 实装细节 + §6.7 ping/pong/shutdown 心跳协议拓扑修正 + §8.4 `MinimalProxy` + §9 PLAN 修订表追加 P1-13。
- [x] 每 task TDD（RED-GREEN-REFACTOR 全程贯穿，每 task 单独 commit，13 提交）
- [x] `pytest tests/ -m "not requires_docker and not requires_api_key"` 在 CI 全绿（**201 pass, 2 deselected**，0 fail）
- [x] mypy + ruff 全清（mypy `Success: no issues found in 67 source files` · ruff `All checks passed!`）
- [ ] `docker build images/gg-relay-runner/` 本地成功（plan 明确说可跳，本地未跑；文件已 ready，Task 0 spike 已证明四件套构建可行）
- [x] `examples/walking_skeleton_demo.py` 仍 exit 0（已实跑确认）
- [x] `examples/docker_executor_demo.py` stub 模式跑通（`DOCKER_AVAILABLE=false`，本环境无 API key 跳过 docker 模式）
- [x] spec / plan template 同步（spec §5.4/§5.5/§6.6/§6.7/§8.4/§9 全部追加；plan §10 全程 `[x]` 同步）
- [ ] subagent-driven-development（本次单 agent 端到端执行，未拆分子 agent；可后续 Plan 4 起再做）

**Coverage（Plan 3 收口数字）**：1225 statements / 97 missed = **92.08%**，
卡在 `proxy/server.py` 86% (CONNECT 异常分支 / plain-HTTP 转发的 IO 失败路径)
和 `wire_runner.py` 42% (signal 处理 / asyncio.run 入口段)；目标 90% 已达
（裕度 +2.08pp），未来 Plan 4 起再补低覆盖的 IO 长尾。
