...
class EventBus:
    ...
    async def replay_after(
        ...
        """Yield durable events with ``events.seq > last_seq`` in order.

        Backs the SSE ``Last-Event-ID: "<seq>:<uuid>"`` cursor
        (Plan 7 D7.17). ``last_seq=None`` (header missing or
        unparseable) or ``durable_store=None`` (bus has no disk
        tier) yields nothing: the SSE generator falls through to the
        live subscriber tail. Otherwise we delegate to
        :meth:`DurableEventStore.fetch_after` and yield each row;
        the SSE renderer formats ids as ``"<seq>:<event_id>"`` so
        the next reconnect resumes from the right place.
        """
    ...
    # ── Plan 9 D9.0 — EventBusBackend Protocol conformance ──────────
    # `subscribe_all` is the cross-worker durable-replay surface the
    # `RedisStreamEventBus` implements natively (XREAD on the
    # gg-relay:events stream). For the in-process bus we delegate to
    # the attached durable_store; if no store is wired, we yield
    ...
    async def subscribe_all(
        ...
        *,
        ...
        :meth:`subscribe`). ``RedisStreamEventBus`` replaces this
        with XREAD ``$`` (read from end) semantics.
        """
